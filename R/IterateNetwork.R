iterateNetwork <- function(net.object,
  ...
  for(u in 1:length(net.samples)) {
    ...
    for(j in 1:net.iterate) {
      ...
      clusters.vec <- c(clusters.vec,igraph::clusters(corenet.gx)$no)
      avr.pathlength.vec <- c(avr.pathlength.vec,igraph::average.path.length(corenet.gx))
      avr.degree.vec <- c(avr.degree.vec,base::mean(igraph::degree(corenet.gx)))
...
<<<<<<< HEAD
    if(estimates.total==28) { plot.panels <- c(4,7) }
=======
    if(estimates.total==28 { plot.panels <- c(4,7) }
>>>>>>> 6ab3bba5ec7a6575b56121130c5c42889bcee16b
    las.plot <- 0
